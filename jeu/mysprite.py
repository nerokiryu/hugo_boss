...
#------------------------Constructeur, self correspond plus ou moins au 'this'------------------------#
	def __init__(self, pos_x, pos_y, image1, image2, image3):
		...
		# On cree un attribut tableau stockant les 3 images
		# que notre objet pourra prendre
		self.images = []
...
#------------------------Change l'image affichee lors d'un mouvement------------------------#
...
#------------------------Change l'image affichee lors d'un arret------------------------#
	# Cette methode parcours notre tableau d'images pour
	# changer l'image affichee de l'objet
	# De plus on utilise un timer pour ne pas changer
	# cette image a chaque iteration de notre boucle de jeu
	def change_img_arret(self):
		if self.timeToUpdate > 2000:
			# On remet le compteur a 0
			self.timeToUpdate = 0
